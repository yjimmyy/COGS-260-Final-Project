...
def control(control_in, question, dim, reuse=None):
    with tf.variable_scope('control', reuse=reuse):
        cq_i = fc(tf.concat([control_in, question], axis=1), dim, activation=None, name='fc_1')
...
def read(control_in, mem_prev, k, dim, n_objs, reuse=None):
    with tf.variable_scope('read', reuse=reuse):
        ...
        I_prime = fc(tf.concat([I, k], axis=1), dim, activation=None)
...
def write(read_in, mem_prev, control_in, dim, reuse=None):
    with tf.variable_scope('write', reuse=reuse):
        m_i_info = fc(tf.concat([read_in, mem_prev], axis=1), dim, activation=None, name='fc_1')
        
        m_i = tf.concat([mem_prev, m_i_info, mem_prev*m_i_info], axis=1)
        m_i = fc(m_i, dim, activation=None, name='fc_2')
...
def train(sess, nbatch, data_train, data_test):
    ...
    #resized_images = tf.image.resize_images(b_images, [128, 128])
    conv_1 = conv2d(b_images, 24, 5, strides=(2,2)) # 64
    conv_2 = conv2d(conv_1, 24, 5, strides=(2,2)) # 32
    conv_3 = conv2d(conv_2, dim, 3, strides=(2,2)) # 16
    conv_3 = conv2d(conv_3, dim, 3, strides=(2,2)) # 8
