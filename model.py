...
def train(sess, nbatch, data_train, data_test):
    ...
    conv_1 = conv2d(resized_images, 24, 5, strides=(2,2)) # 32
    ...
    with tf.variable_scope('RNN'):
        for time_step in range(num_steps):
            ...
            fc_1 = fc(tf.concat([conv_3, b_questions, cell_output], axis=1), 32, name='fc_1')
            fc_2 = fc(fc_1, 32, name='fc_2')

            x_t = fc_2
            cell_output, state = cell(x_t, state)
            cell_output = cell_output + x_t
...
def main():
    ...
    with tf.Session() as sess:
        ...
        train(sess, 50, rel_train, rel_test)
