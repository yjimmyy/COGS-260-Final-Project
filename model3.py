...
def train(sess, nbatch, data_train, data_test):
    ...
    conv_1 = conv2d(resized_images, 10, 7, strides=(2,2)) # 32
    conv_2 = conv2d(conv_1, 10, 5, strides=(2,2)) # 16
    conv_3 = conv2d(conv_2, 10, 3, strides=(2,2)) # 8
    ...
    with tf.variable_scope('RNN'):
        for time_step in range(num_steps):
            ...
            fc_1 = fc(tf.concat([cell_output, b_questions], axis=1), 64, name='fc_1')
            fc_2 = fc(fc_1, 64, name='fc_2')
            att = tf.tile(tf.reshape(fc_1, [-1,8,8,1]), [1,1,1,10])
            conv_att = tf.multiply(conv_3, att)
            conv_att = tf.reduce_sum(conv_att, axis=[1,2])
            conv_att = tf.layers.flatten(conv_att)
            fc_3 = fc(conv_att, 64, name='fc_3', activation=None)

            """
            fc_1 = fc(tf.concat([conv_3, b_questions, cell_output], axis=1), 64, name='fc_1')
            fc_2 = fc(fc_1, 32 , name='fc_2')
            fc_2 = tf.layers.dropout(fc_2, rate=0.4)
            fc_3 = fc(fc_2, 32, name='fc_3')
            """
    ...
    loss_coefs = [1.2, 0.8, 0.4, 0.3, 0.2, 0.2]
...
def main():
    ...
    with tf.Session() as sess:
        #train(sess, 50, norel_train, norel_test)
